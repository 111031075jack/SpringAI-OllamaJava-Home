...
public class QueryGUI extends JFrame {
	...
	private void initUI() {
		setTitle("我的 AI 財經顧問");
		setDefaultCloseOperation(EXIT_ON_CLOSE); // 按下關閉及系統結束
		setSize(700, 550); // 視窗尺寸
		
		// 建立用於 UI 排版的 JPanel, 並使用 GridBagLayout 版面配置管理器
		JPanel formPanel = new JPanel(new GridBagLayout());
		
		// 建立 GridBagConstraints 用於控制元件在 GridBagLayout 管理器中的放置細節
		GridBagConstraints gbc = new GridBagConstraints();
		
		// 設定元件四周的邊距為 8 像素(上, 下, 左, 右)
		gbc.insets = new Insets(8, 8, 8, 8);
		
		// 放置元件時位置以元件的左上角為對其基準點
		...
		resultScore.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS); // 總是顯示垂直滾動條
		
		// 創建 "警語" 元件 --------------------------------------------
		JLabel footLabel = new JLabel("** 投資之前，應該充分了解相關公告等相關信息，以便做出明智的投資決策 **");
		resultArea.setFont(new Font("sansserif", Font.PLAIN, 14)); // 設定字體, 風格與大小
		
		// 將 formPanel 放在 QueryGUI 主畫面的上方
		add(formPanel, BorderLayout.NORTH);
		
		// 將 resultScore 放在 QueryGUI 主畫面中央
		add(resultScore, BorderLayout.CENTER);
		
		// 將 resultScore 放在 QueryGUI 主畫面下方
		add(footLabel, BorderLayout.SOUTH);
